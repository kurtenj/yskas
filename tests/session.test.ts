...
import { requireSession, SESSION_COOKIE, signToken, verifySession } from "../lib/session";
...
vi.mock("../lib/server-limits", () => ({ acquireLimit: limits.acquire, releaseLimit: limits.release }));
let privateJwk: string;
let jwks: string;
beforeAll(async () => {
  const pair = await generateKeyPair("RS256", { extractable: true });
  privateJwk = JSON.stringify({ ...await exportJWK(pair.privateKey), kid: "test" });
  jwks = JSON.stringify({ keys: [{ ...await exportJWK(pair.publicKey), kid: "test", alg: "RS256", use: "sig" }] });
});
beforeEach(() => {
...
function request(token?: string, origin?: string) {
  return new NextRequest("https://yskas.test/api/auth/token", { method: "POST", headers: {
    ...(token ? { cookie: `${SESSION_COOKIE}=${token}` } : {}), ...(origin ? { origin } : {}),
  } });
}
test("valid signed sessions exchange for short-lived Convex-only tokens", async () => {
  ...
  const { payload } = await jwtVerify(token, createLocalJWKSet(JSON.parse(jwks)), { audience: "convex" });
...
test("forged, expired, wrong-role, wrong-audience and revoked sessions fail", async () => {
  ...
  for (const value of [undefined, "true", valid + "x", await signToken("household", "yskas-session", -1), await signToken("service", "yskas-session", 60), await signToken("household", "convex", 60)]) {
...
test("foreign origin is rejected even with a valid session", async () => {
  const session = await signToken("household", "yskas-session", 60);
  expect((await requireSession(request(session, "https://evil.test")))?.status).toBe(403);
});
test("old literal cookie and absent sessions cannot reach paid providers or quotas", async () => {
  for (const handler of [estimate, transcribe, exchange]) {
    const response = await handler(new NextRequest("https://yskas.test/api/test", { method: "POST", headers: { cookie: "pin_verified=true" } }));
...
test("PIN login signs an HttpOnly cookie only after durable limit admission", async () => {
  const pinRequest = () => new NextRequest("https://yskas.test/api/verify-pin", { method: "POST", headers: { "content-type": "application/json" }, body: '{"pin":"1234"}' });
  const response = await login(pinRequest());
  expect(response.status).toBe(200);
  expect(response.headers.get("set-cookie")).toContain("HttpOnly");
  const cookie = response.headers.get("set-cookie")?.match(/yskas_session=([^;]+)/)?.[1];
  expect(await verifySession(cookie)).toBe(true);
  limits.acquire.mockResolvedValueOnce({ response: Response.json({ error: "Limited" }, { status: 429 }) });
