...
- Calories use kcal; protein, fiber, carbs and fat use grams. Shared validation rejects negative/nonfinite values and blank or oversized text. Decimals are retained; display rounds to the nearest whole number.
...
- Missing nutrients remain unknown. Partial daily amounts show the known gram total without extra completeness subtext. Missing meal nutrients display a dash. An empty day has zero consumed grams. The chart caps at the calorie goal; nutrient text retains over-goal amounts.
